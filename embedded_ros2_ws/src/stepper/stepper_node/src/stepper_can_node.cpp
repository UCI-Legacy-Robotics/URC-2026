...
StepperCanNode::StepperCanNode(const std::string& node_name) : rclcpp::Node(node_name) {
    ...
    odrv_publisher_ = rclcpp::Node::create_publisher<ODriveStatus>("stepper_status", odrv_stat_qos);

    rclcpp::QoS ctrl_msg_qos(rclcpp::KeepAll{});
    subscriber_ = rclcpp::Node::create_subscription<ControlMessage>("control_message", ctrl_msg_qos, std::bind(&StepperCanNode::subscriber_callback, this, _1));
    ...
    service_ = rclcpp::Node::create_service<AxisState>("request_axis_state", std::bind(&StepperCanNode::service_callback, this, _1, _2), srv_qos_profile);
    service_clear_errors_ = rclcpp::Node::create_service<Empty>("clear_errors", std::bind(&StepperCanNode::service_clear_errors_callback, this, _1, _2), srv_qos_profile);
}

void StepperCanNode::deinit() {
...
bool StepperCanNode::init(EpollEventLoop* event_loop) {
    ...
    if (!can_intf_.init(interface, event_loop, std::bind(&StepperCanNode::recv_callback, this, _1))) {
        RCLCPP_ERROR(rclcpp::Node::get_logger(), "Failed to initialize socket can interface: %s", interface.c_str());
        return false;
    }
    if (!sub_evt_.init(event_loop, std::bind(&StepperCanNode::ctrl_msg_callback, this))) {
        RCLCPP_ERROR(rclcpp::Node::get_logger(), "Failed to initialize subscriber event");
        return false;
    }
    if (!srv_evt_.init(event_loop, std::bind(&StepperCanNode::request_state_callback, this))) {
        RCLCPP_ERROR(rclcpp::Node::get_logger(), "Failed to initialize service event");
        return false;
    }
    if (!srv_clear_errors_evt_.init(event_loop, std::bind(&StepperCanNode::request_clear_errors_callback, this))) {
...
void StepperCanNode::recv_callback(const can_frame& frame) {
    ...
    switch(frame.can_id & 0x1F) {
        case CmdId::kHeartbeat: {
            ...
            // Values below not used by stepper
            ctrl_stat_.pos_estimate = 0;
            ctrl_stat_.vel_estimate = 0;
            ctrl_stat_.iq_setpoint = 0;
            ctrl_stat_.iq_measured = 0;
            ctrl_stat_.torque_target   = 0;
            ctrl_stat_.torque_estimate = 0;
            ctrl_pub_flag_ |= 0b1111;

            // Publish fake odrive stat data for testing
            odrv_stat_.active_errors = 0;
            odrv_stat_.disarm_reason = 0;
            odrv_stat_.fet_temperature   = 0;
            odrv_stat_.motor_temperature = 0;
            odrv_stat_.bus_voltage = 0;
            odrv_stat_.bus_current = 0;
            odrv_pub_flag_ |= 0b111;
...
void StepperCanNode::subscriber_callback(const ControlMessage::SharedPtr msg) {
    std::lock_guard<std::mutex> guard(ctrl_msg_mutex_);
    ctrl_msg_ = *msg;
    sub_evt_.set();
}

void StepperCanNode::service_callback(const std::shared_ptr<AxisState::Request> request, std::shared_ptr<AxisState::Response> response) {
    {
...
void StepperCanNode::service_clear_errors_callback(const std::shared_ptr<Empty::Request> request, std::shared_ptr<Empty::Response> response) {
    RCLCPP_INFO(rclcpp::Node::get_logger(), "clearing errors");
    srv_clear_errors_evt_.set();
}

void StepperCanNode::request_state_callback() {
    uint32_t axis_state;
    {
        std::unique_lock<std::mutex> guard(axis_state_mutex_);
...
void StepperCanNode::request_clear_errors_callback() {
    struct can_frame frame;
    frame.can_id = node_id_ << 5 | CmdId::kClearErrors;
    write_le<uint8_t>(0, frame.data);
    frame.can_dlc = 1;
...
void StepperCanNode::ctrl_msg_callback() {
    ...
    switch (control_mode) {
        ...
        case ControlMode::kStepperControl: {
            RCLCPP_DEBUG(rclcpp::Node::get_logger(), "input_step_vel");
            frame.can_id = node_id_ << 5 | kSetInputVel;
            std::lock_guard<std::mutex> guard(ctrl_msg_mutex_);
...
inline bool StepperCanNode::verify_length(const std::string&name, uint8_t expected, uint8_t length) {
