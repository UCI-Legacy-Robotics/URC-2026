...
ServoCanNode::ServoCanNode(const std::string& node_name) : rclcpp::Node(node_name) {
    ...
    subscriber_ = rclcpp::Node::create_subscription<ControlMessage>("control_message", ctrl_msg_qos, std::bind(&ServoCanNode::subscriber_callback, this, _1));

}

void ServoCanNode::deinit() {
    sub_evt_.deinit();
    can_intf_.deinit();
}

bool ServoCanNode::init(EpollEventLoop* event_loop) {

    node_id_ = rclcpp::Node::get_parameter("node_id").as_int();
    std::string interface = rclcpp::Node::get_parameter("interface").as_string();

    if (!can_intf_.init(interface, event_loop, std::bind(&ServoCanNode::recv_callback, this, _1))) {
        RCLCPP_ERROR(rclcpp::Node::get_logger(), "Failed to initialize socket can interface: %s", interface.c_str());
        return false;
    }
    if (!sub_evt_.init(event_loop, std::bind(&ServoCanNode::ctrl_msg_callback, this))) {
...
void ServoCanNode::subscriber_callback(const ControlMessage::SharedPtr msg) {
    std::lock_guard<std::mutex> guard(ctrl_msg_mutex_);
    ctrl_msg_ = *msg;
    sub_evt_.set();
}

void ServoCanNode::ctrl_msg_callback() {
...
void ServoCanNode::recv_callback(const can_frame& frame) {
    // Don't do anything since servo only gives back heartbeat
    // Look at heartbeat and publish later
}

inline bool ServoCanNode::verify_length(const std::string&name, uint8_t expected, uint8_t length) {
